...
namespace RPGMadeEasy
{
    public class Menu
    {
        ...
        public string ID 
        {
            ...
            set
            {
                id = value; // Once we set an id EventHandler is gonna call this event
        ...
        void AlignMenuItems()
        {
            ...
            dimensions = new Vector2((ScreenManager.Instance.Dimensions.X - // where we need to draw in order to
                dimensions.X) / 2, (ScreenManager.Instance.Dimensions.Y - dimensions.Y) / 2); // centrilize our text
        ...
        {
            string[] split = Effects.Split(':'); // set a universe effect for menu item
            foreach (MenuItem item in Items)
            {
